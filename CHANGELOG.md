...
- 新增 `World` 地点注册、统一二维坐标与多边形包含查询；支持负坐标、父子地点和地点内移动。Passage 使用已注册英文 ID 与 `inside` / `outside` 标签，开始菜单保持未定位。
- 世界位置接入事务回滚、历史和存档；存档格式升级至 v3，读取 v2 时默认为未定位。读档和语言刷新保持当前坐标，刷新中的新导航恢复正常移动。
- 修复 Reaction 条件与动态事件载荷回调中查询规则可能触发借用冲突的问题，并保留生命周期规则发出事件时的载荷。
- 隔离作者 TypeScript 项目的全局类型，修复 `setup` 被识别为 Mocha 测试钩子、`Event` 与 DOM 类型冲突的问题；独立类型包包含完整契约声明。
- VS Code 宏、表达式和 TypeScript API 提供中文在前、英文在后的双语说明；整理作者指南与源码索引，补充世界地图示例。
