...
fn registration_accepts_forward_parents_and_uses_ids() {
    ...
    location.add(hospital).unwrap();
    assert!(location.validate().is_err());
    location.add(square("town", None, 0, 10)).unwrap();
    location.validate().unwrap();
    assert_eq!(
        location.get("town-hospital").unwrap().name.as_deref(),
        Some("医院")
    );
    assert!(location.get("医院").is_none());
    assert_eq!(location.places().len(), 2);
    assert_eq!(
        location
...
fn registration_rejects_reserved_and_invalid_ids_without_replacing_places() {
    ...
    for id in [
        ...
        assert!(location.add(square(id, None, 0, 10)).is_err(), "{id}");
    }
    location
    ...
    assert_eq!(
        location.get("Town.hospital_1-wing").unwrap().bounds[0],
        [0, 0]
    );
}
...
fn polygons_reject_degeneracy_intersections_and_unsafe_coordinates() {
    ...
    for bounds in invalid {
        ...
        place.bounds = bounds;
        assert!(location.add(place).is_err());
        assert_eq!(location.places().len(), 0);
    }
}
...
fn geometry_accepts_concavity_collinear_edges_and_boundary_points() {
    ...
    location.add(place).unwrap();
    location.validate().unwrap();
    assert_eq!(location.locate([2, 2]).len(), 1);
    assert_eq!(location.locate([1, 1]).len(), 1);
    assert!(location.locate([2, 3]).is_empty());
    assert!(location.locate([MAX_COORDINATE + 1, 0]).is_empty());
}
...
fn geometry_is_exact_near_safe_integer_limits() {
    let mut location: Location = Location::default();
    location
        .add(square("location", None, -MAX_COORDINATE, MAX_COORDINATE))
        .unwrap();
    assert_eq!(location.locate([MAX_COORDINATE, MAX_COORDINATE]).len(), 1);
    assert_eq!(location.locate([-MAX_COORDINATE, MAX_COORDINATE]).len(), 1);
    assert_eq!(location.locate([0, 0]).len(), 1);
}

#[test]
fn movement_supports_negative_places_and_crossing_the_origin() {
    let mut location: Location = Location::default();
    location.add(square("town", None, -10, 10)).unwrap();
    let mut cellar: Place = square("cellar", Some("town"), -8, -2);
    cellar.entry = Some([-6, -4]);
    location.add(cellar).unwrap();
    location.validate().unwrap();

    let mut state: LocationState = LocationState::default();
    location.enter(&mut state, "cellar", None).unwrap();
    assert_eq!(state.position.as_ref().unwrap().point, [-6, -4]);
    assert_eq!(location.locate([-6, -4]).len(), 2);
    location.enter(&mut state, "town", None).unwrap();
    for point in [[0, 0], [-3, 2], [4, -1]] {
        assert!(location.move_to(&mut state, point).unwrap());
        assert_eq!(state.position.as_ref().unwrap().point, point);
        location.validate_state(&state).unwrap();
    }
}
...
fn negative_safe_integer_boundary_is_inclusive_without_signed_overflow() {
    ...
    location
        ...
        .unwrap();
    let mut state: LocationState = LocationState::default();
    location.enter(&mut state, "location", None).unwrap();
    assert_eq!(
        state.position.as_ref().unwrap().point,
        [-MAX_COORDINATE, -MAX_COORDINATE]
    );
    location.validate_state(&state).unwrap();
    let before: LocationState = state.clone();

    for coordinate in [-MAX_COORDINATE - 1, i64::MIN] {
        let point: Point = [coordinate, 0];
        let invalid: Place = square("invalid", None, coordinate, 0);
        assert_eq!(
            location.add(invalid).unwrap_err().code(),
            "location.invalid_bounds"
        );
        assert!(location.locate(point).is_empty());
        assert_eq!(
            location.move_to(&mut state, point).unwrap_err().code(),
            "location.invalid_position"
        );
        assert_eq!(state, before);
        let invalid: LocationState = LocationState {
            position: Some(LocationPosition {
                place: "location".to_owned(),
                point,
                environment: None,
            }),
        };
        assert_eq!(
            location.validate_state(&invalid).unwrap_err().code(),
            "location.invalid_position"
        );
    }
}
...
fn validation_rejects_parent_cycles_and_child_edges_outside_concave_parent() {
    let mut cycle: Location = Location::default();
...
fn locate_is_stable_parent_first_and_retains_overlaps() {
    let mut location: Location = Location::default();
    location.add(square("shop", Some("town"), 4, 9)).unwrap();
    location
        .add(square("hospital", Some("town"), 1, 8))
        .unwrap();
    location.add(square("town", None, 0, 10)).unwrap();
    location.validate().unwrap();
    assert_eq!(
...
fn enter_preserves_same_place_position_and_movement_rejects_without_mutation() {
    ...
    location.add(hospital).unwrap();
    let mut state: LocationState = LocationState::default();
    assert!(location.move_to(&mut state, [1, 1]).is_err());
    assert!(
        location
    ...
    assert!(location.move_to(&mut state, [5, 5]).unwrap());
    assert!(!location.enter(&mut state, "town", None).unwrap());
    ...
    assert!(location.enter(&mut state, "hospital", None).unwrap());
    assert_eq!(state.position.as_ref().unwrap().point, [3, 3]);
    assert_eq!(state.position.as_ref().unwrap().environment, None);
    let before: LocationState = state.clone();
    assert!(location.move_to(&mut state, [0, 0]).is_err());
    assert!(location.enter(&mut state, "missing", None).is_err());
    assert_eq!(state, before);
    assert!(!location.move_to(&mut state, [3, 3]).unwrap());
    location.validate_state(&state).unwrap();
}
...
fn state_and_entry_validation_include_boundaries_and_reject_invalid_locations() {
    ...
    assert!(location.add(invalid).is_err());
    location.add(square("town", None, 0, 10)).unwrap();
    location.validate_state(&LocationState::default()).unwrap();
    for (id, point) in [
        ("missing", [1, 1]),
        ("town", [11, 1]),
