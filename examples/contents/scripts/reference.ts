...
Macro.add("barDemo", {
  ...
  execution: "sync",
  handler: () => {
    if (V.demo_screen !== "reference") {
      return Macro.get("townStatus")!.handler({ name: "townStatus", arguments: "" })
    }
...
Macro.add("barStowedDemo", {
  ...
  handler: () => {
    if (V.demo_screen !== "reference") {
      return Macro.get("townCompact")!.handler({ name: "townCompact", arguments: "" })
    }
    const english = I18n.locale === "en"
    return Surface.fragment(
