...
Macro.add("hostTools", {
  ...
  arguments: "raw",
  execution: "sync",
  handler: () => Presentation.component(
    "narrava.host-tools",
    1,
    {},
    ["存档、语言与日志由当前 Host 按能力显示。"],
    { key: "host-tools" },
  ),
})
...
Macro.add("presentationDemo", {
  ...
  handler: () => Presentation.fragment(
    ...
    Presentation.region("bar", [
      Presentation.text("测试工具", { key: "bar-heading", styles: ["heading2"] }),
      Presentation.component("meter", 1, {
        label: "探索进度",
        value: 72,
        min: 0,
        max: 100,
      }, ["探索进度：72 / 100"], { key: "exploration-meter" }),
    ], { key: "demo-bar" }),
    ...
    Presentation.component("future-card", 1, { title: "未知组件" }, [
      Presentation.text("Host 不认识该组件，因此显示这段 fallback。", { tone: "muted" }),
    ], { key: "fallback-demo" }),
    Presentation.region("footer", [
...
Macro.add("dialogDemo", {
  ...
  handler: () => Presentation.fragment(
    ...
    Presentation.region("dialog", [
      ...
      Presentation.action("次要按钮", "dismiss", { key: "secondary-action", role: "secondary" }),
