...
//! `narrava-loom-protocol`；Host 消费前由 Script Runtime 直接映射为 Protocol DTO。
//! Script builder 使用的 Surface 只负责校验不可信脚本输出。
...
impl fmt::Display for SemanticKeyError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Empty => formatter.write_str("SemanticOutput key 不能为空"),
            Self::Duplicate(key) => write!(formatter, "SemanticOutput key 重复：{key}"),
        }
    }
}

impl Error for SemanticKeyError {}

impl SemanticKey {
    /// 校验并构造 key；空字符串被拒绝。
    pub fn parse(key: impl Into<String>) -> Result<Self, SemanticKeyError> {
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SemanticInputBinding {
    pub receiver: String,
    pub kind: SemanticInputKind,
}

impl SemanticInputBinding {
    ...
    pub fn accepts(&self, value: &SemanticValue) -> bool {
        match &self.kind {
            ...
            SemanticInputKind::Text { .. } => matches!(value, SemanticValue::Text(_)),
        }
    }
}
...
impl ComponentCapability {
    /// 校验并构造能力名；空字符串被拒绝。
    pub fn parse(value: impl Into<String>) -> Result<Self, SemanticKeyError> {
        let value: String = value.into();
        if value.is_empty() {
            return Err(SemanticKeyError::Empty);
        }
...
impl InteractionId {
    ...
    /// 从 Binding 可传输的字符串恢复身份；权限仍由 SemanticOutput 验证。
...
pub struct SemanticOutput {
    ...
    keys: Vec<Option<SemanticKey>>,
}

impl SemanticOutput {
    /// 从已经排好顺序的语义节点建立输出。
    pub fn from_nodes(nodes: Vec<SemanticNode>) -> Self {
        let keys: Vec<Option<SemanticKey>> = vec![None; nodes.len()];
        Self { nodes, keys }
    }
    ...
    pub fn push_keyed(
        ...
        {
            return Err(SemanticKeyError::Duplicate(key.0));
        }
        self.nodes.push(node);
    ...
    pub fn interaction_target(&self, id: &InteractionId) -> Option<&str> {
        self.nodes
            .iter()
            .find_map(|node: &SemanticNode| match node {
    ...
    pub fn input_binding(&self, id: &InteractionId) -> Option<&SemanticInputBinding> {
        self.nodes
            .iter()
            .find_map(|node: &SemanticNode| match node {
                SemanticNode::Input {
