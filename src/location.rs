...
pub use narrava_loom_location::{
    Environment, Location, LocationError, LocationPosition, LocationState, MAX_COORDINATE, Place,
    Point,
};
...
pub fn passage_place<'location>(
    location: &'location Location,
    passage: &HirPassage<'_>,
) -> Result<(Option<&'location Place>, Option<Environment>), Diagnostic> {
    let mut place: Option<&Place> = None;
    for tag in &passage.tags {
        if let Some(candidate) = location.get(tag) {
...
pub fn validate_passages(location: &Location, story: &HirStory<'_>) -> Result<(), Diagnostic> {
    for passage in &story.passages {
        let _binding = passage_place(location, passage)?;
    }
    Ok(())
}
...
pub fn enter_passage(state: &mut State, passage: &HirPassage<'_>) -> Result<(), Diagnostic> {
    if passage.name == START_PASSAGE {
        *state.location_state_mut() = LocationState::default();
        return Ok(());
    }
    let (place, environment) = passage_place(state.location(), passage)?;
