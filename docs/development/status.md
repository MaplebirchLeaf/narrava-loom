...
| 工作 | 状态 | 完成标准 |
| --- | --- | --- |
| Audio 生命周期与 tag | 已实现 | Header/Footer 公共声明、正文覆盖、连续播放与自动停止；见 [Audio](../author/audio.md) |
...
| 显式弹窗宏与 page 子句 | 已实现 | 无导航打开、默认页、任意页数、关闭与重开；见[弹窗](../author/dialog.md) |
| 桌面发行目录回归 | 待真实窗口验收 | 图片、输入、I18n、Save、缩放与键盘操作 |

下一步集中进行桌面发行目录验收；Audio 生命周期与 tag 匹配已实现，真实音频设备验收需与自动测试分别记录。Model、模组和新平台暂不展开。
