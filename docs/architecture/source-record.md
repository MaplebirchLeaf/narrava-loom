...
| `random`                       | 可存档、可回放的固定随机序列                                                               |
| `inspect`                      | 有界 State 值图只读预览                                                                    |
| `location`                     | Location 类型重导出、Passage 地点与环境 Tag 绑定                                           |
...
| `state_adapter.rs` / `location_adapter.rs`    | 活动 State 访问与 Location 注册                          |
| `random_adapter.rs` / `refresh.rs`            | 共享随机源与当前页重绘的状态隔离                         |
| `logger_adapter.rs`                           | 作者 Logger 与宿主统一日志桥接                           |
...
| `crates/narrava-loom-location/`           | 地点定义、二维多边形与位置规则，独立于编译器和宿主         |
