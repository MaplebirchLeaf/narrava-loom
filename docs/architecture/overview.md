...
6. 本体、I18n 与 Script Bundle 位于 `narrava-loom-core` crate；Surface 协议语义与跨 Host 传输层位于 `narrava-loom-protocol` crate（依赖 Core，Core 内部使用 `semantic` 语义输出类型）；ECMAScript 游戏脚本执行与宏分发位于 `narrava-loom-script` crate（依赖 Protocol）；ModLoader 是独立项目，不计入 Core。依赖只能沿 `host / ModLoader → narrava-loom-script → narrava-loom-protocol → Core` 方向，反向不可达。
